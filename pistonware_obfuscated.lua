...
-- ENTROPY: 0x00158acb | CHECKSUM: VERIFIED
...
shared.PistonwareRelease = {
    ...
    branch = "main",
    sourceRef = "main",
    version = "main@standalone-monolith",
    cacheReady = true,
    resolved = true
}
...
local function downloadFile(path, func)
	...
	local relPath = select(1, path:gsub('^pistonware/', ''))
	for _, candidate in ipairs({path, relPath, 'pistonware/' .. relPath}) do
		if isfile and isfile(candidate) then
			local ok, content = pcall(readfile, candidate)
			if ok and type(content) == 'string' and content ~= '' then
				return func and func(candidate) or content
			end
		end
	end
...
local function loadModule(path, name)
	if PistonwareModules and PistonwareModules[path] then
		return PistonwareModules[path]
	end
...
if vape and type(vape.Load) == "function" then
    ...
    if type(vape.CreateNotification) == "function" then
        vape:CreateNotification((string.char(80, 105, 115, 116, 111, 110, 119, 97, 114, 101, 32, 83, 116, 97, 110, 100, 97, 108, 111, 110, 101)), "Vape V4 Standalone initialized successfully. Zero loaders, zero keys.", 5, "info")
    end
end;
